...
class API {
  ...
  // Create the envelope from given parameters
  String makeEnvelope(Map<String, dynamic> items, bool isRequest) {
    final List<String> envelope = [];

    envelope.add(getEnvelopeLogin());
    // Add prefix to request
    if (isRequest) envelope.add('<web:request>');

    // Add items to envelope

    // Add suffix to request
    if (isRequest) envelope.add('</web:request>');
